...
const renderTable = () => {
  ...
  <td class="shrink" data-column="date" data-label="Date">${filterLink('date', item.date, 'var(--text-primary)', item.date ? formatDisplayDate(item.date) : '—')}</td>
      ...
      <td class="icon-col actions-cell" data-column="actions" data-label=""><div class="actions-row">
        ...
        <button class="icon-btn action-icon" role="button" tabindex="0" onclick="typeof showClonePicker==='function'?showClonePicker(${originalIdx}):duplicateItem(${originalIdx})" aria-label="Clone ${sanitizeHtml(item.name)}" title="Clone item">
