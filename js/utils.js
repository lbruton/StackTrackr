...
const updateStorageStats = async () => {
  try {
    let used = 0;
    let quota = 5 * 1024 * 1024; // default 5MB limit

    if (navigator?.storage?.estimate) {
      const estimate = await navigator.storage.estimate();
      used = estimate.usage || used;
      quota = estimate.quota || quota;
    } else {
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        const value = localStorage.getItem(key);
        // localStorage stores strings in UTF-16 (~2 bytes per char)
    ...
    if (el) {
      el.textContent = `${usedKB.toFixed(1)} KB / ${quotaKB.toFixed(1)} KB`;
    }
...
<body>
    <div class="report-container">
        ...
        <section class="storage-chart">
            <h2>Top Storage Items</h2>
        ...
        const options = {
            ...
            scales: {
                x: {
                    ...
                    grid: { color: isDark ? '#404040' : '#e0e0e0' }
                },
                y: {
                    beginAtZero: true,
                    ticks: { color: isDark ? '#ffffff' : '#000000' },
