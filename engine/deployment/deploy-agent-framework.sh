...
# Configuration
SOURCE_PROJECT="/Volumes/DATA/GitHub/StackTrackr/engine/templates"
TARGET_PROJECT="${1:-}"
...
# Core Framework Files to Copy (from templates directory)
FRAMEWORK_FILES=(
    "agents.ai:agents/agents.ai"
    "QUICK-AGENT-PROTOCOLS.ai:agents/QUICK-AGENT-PROTOCOLS.ai"
    "unified-workflow.ai:agents/unified-workflow.ai"
    "evening-work-session-protocol.md:docs/evening-work-session-protocol.md"
    "mcp_backup_system.py:scripts/mcp_backup_system.py"
    "backup_mcp_memory.sh:scripts/backup_mcp_memory.sh"
    "sync_memory.sh:scripts/sync_memory.sh"
...
for file_pair in "${FRAMEWORK_FILES[@]}"; do
    source_file="${file_pair%:*}"
    target_file="${file_pair#*:}"
    
    if [ -f "$SOURCE_PROJECT/$source_file" ]; then
        # Create target directory if needed
        mkdir -p "$(dirname "$TARGET_PROJECT/$target_file")"
        cp "$SOURCE_PROJECT/$source_file" "$TARGET_PROJECT/$target_file"
