...
// Evaluate the file directly since Node's ES modules might not play nice with
// the manual conditional module.exports in the file.
const sourceCode = fs.readFileSync(path.join(__dirname, '../js/fuzzy-search.js'), 'utf8');

function getFuzzyMatch() {
  const scope = {};
  // Mock window and module
  const fn = new Function('window', 'module', sourceCode + '\nreturn fuzzyMatch;');
  return fn(scope, { exports: {} });
}

const fuzzyMatch = getFuzzyMatch();
