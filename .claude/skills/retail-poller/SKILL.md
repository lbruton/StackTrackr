...
**Dual-poller write-through:** Both Fly.io (`POLLER_ID=api`) and Portainer VM (`POLLER_ID=home`) write to the same Turso DB. `api-export.js` uses `readLatestPerVendor()` to merge — most recent row per vendor within last 2h wins. Check home poller logs via Portainer API — see `portainer` skill.
